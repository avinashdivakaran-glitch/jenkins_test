...
var (
	...
	clientCertFile = "/app/certs/tnn-wifi_app.crt"
	clientKeyFile  = "/app/certs/tnn-wifi_app.key"
)

var (
	topics = map[string]string{
		"service":    "tnn_server/wifi/service",
		"adapter":    "tnn_server/wifi/adapter",
		"property":   "tnn_server/wifi/property",
		"hotspot":    "tnn_server/wifi/hotspot",
		"scan":       "tnn_server/wifi/func/scan",
		"connect":    "tnn_server/wifi/func/connect",
		"disconnect": "tnn_server/wifi/func/disconnect",
		"scan_dev":   "tnn_server/wifi/device/scanned",
	}
)
...
func Subscribe_mqttBroker(mqttClient mqtt.Client, tnn_wifiStatus *wifi_models.WiFiStatus) {
	for _, topic := range topics {
		mqttClient.Subscribe(topic, 0, func(client mqtt.Client, msg mqtt.Message) {
			handleMessage(msg, tnn_wifiStatus)
		})
	}
}

func handleMessage(msg mqtt.Message, tnn_wifiStatus *wifi_models.WiFiStatus) {
	topic := msg.Topic()

	switch topic {
	case topics["adapter"]:
		wifi_mqtt.Handle_adapterSub(tnn_wifiStatus, msg.Payload())
	case topics["hotspot"]:
		wifi_mqtt.Handle_hotspotSub(tnn_wifiStatus, msg.Payload())
	case topics["scan"]:
		wifi_mqtt.Handle_scanSub(tnn_wifiStatus, msg.Payload())
	case topics["connect"]:
		wifi_mqtt.Handle_connectSub(tnn_wifiStatus, msg.Payload())
	case topics["disconnect"]:
		wifi_mqtt.Handle_disconnectSub(tnn_wifiStatus, msg.Payload())
	}
}
